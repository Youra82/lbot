...
# ... (Alle Imports und der BenchmarkCallback bleiben unverändert) ...
...
SCALER = None
SETTINGS = None
# NEU: Globale Variable für den Modus
OPTIM_MODE = "strict" 
...
def objective(trial):
    try:
        ...
        # NEU: Logik zur Anwendung der Constraints je nach Modus
        if OPTIM_MODE == "strict":
            constraints = opti_settings.get('constraints', {})
            min_trades = 20 # Harte untere Grenze für statistische Relevanz
            if (metrics['max_drawdown_pct'] > constraints.get('max_drawdown_pct', 99) or
                ...
                return -999.0
        else: # Modus "find_best"
            if metrics['max_drawdown_pct'] > 80 or metrics['num_trades'] < 5: # Nur sehr lockere Grundregeln
                return -999.0
...
def run_optimization_for_pair(symbol, timeframe, start_date, trials, jobs):
    # ... (Diese Funktion bleibt bis zum Speichern der Config unverändert) ...
    ...
    if not study.best_trial or study.best_value <= -999: # Geändert, um sicherzustellen, dass es nicht nur -999 ist
        logging.warning(f"Optuna fand keine profitable Lösung für {symbol} ({timeframe}), die die Bedingungen erfüllt.")
        return None
    ...
    # ... (Die Erstellung der 'final_config' und das Speichern bleiben unverändert) ...
    final_config = {
        ...
        "behavior": {"use_longs": True, "use_shorts": False},
        "filters": {"ema_period": best_params_dict['ema_period'], "atr_period": best_params_dict['atr_period']}
    }
...
def main():
    ...
    OPTIM_MODE = args.mode # Setze den globalen Modus
