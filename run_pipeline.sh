...
# ... (Alle Abfragen für Symbole, Zeitfenster, Rückblick, Trials, CPU-Kerne bleiben unverändert) ...
...
if [[ "$INPUT_LOOKBACK" == "a" || "$INPUT_LOOKBACK" == "A" ]]; then
    echo -e "${YELLOW}Berechne optimalen Daten-Rückblick...${NC}"; LOOKBACK=0
    for tf in $TIMEFRAMES; do
        current_lookback=0
        case $tf in
            5m) current_lookback=30;; 15m) current_lookback=90;; 30m) current_lookback=180;; 1h) current_lookback=365;; 2h) current_lookback=540;; 4h) current_lookback=730;; 6h) current_lookback=1095;; 1d) current_lookback=1825;; *) current_lookback=$DEFAULT_LOOKBACK;;
        esac
        if [[ $current_lookback -gt $LOOKBACK ]]; then LOOKBACK=$current_lookback; fi
    done
...
# NEU: Abfrage des Optimierungs-Modus
echo -e "\n${CYAN}--- Wähle einen Optimierungs-Modus ---${NC}"
echo "  1) Standard (Nutzt die 'constraints' aus settings.json)"
echo "  2) Finde das Beste (Ignoriert alle Regeln außer max. Drawdown von 80%)"
read -p "Auswahl [1 oder 2, Standard: 1]: " MODE_CHOICE
OPTIM_MODE="strict" # Standardwert
if [[ "$MODE_CHOICE" == "2" ]]; then
    OPTIM_MODE="find_best"
fi
...
echo -e "   - Optim.-Modus:     ${CYAN}${OPTIM_MODE}${NC}"
echo -e "${YELLOW}-------------------------------------------------------${NC}\n"
...
"$VENV_PYTHON" "$TRAINER" --symbols "$SYMBOLS" --timeframes "$TIMEFRAMES" --start_date "$START_DATE"
if [ $? -ne 0 ]; then echo -e "\n${RED}Fehler im Trainer-Skript. Pipeline wird abgebrochen.${NC}"; deactivate; exit 1; fi

echo -e "\n${BLUE}>>> STUFE 2/2: Starte Handelsparameter-Optimierung... <<<${NC}"
if [[ "$JOBS" -ne 1 ]]; then
    display_jobs=$JOBS; if [[ "$JOBS" -eq -1 ]]; then display_jobs="allen"; fi
...
# NEU: Übergabe des Modus an den Optimizer
"$VENV_PYTHON" "$OPTIMIZER" --symbols "$SYMBOLS" --timeframes "$TIMEFRAMES" --start_date "$START_DATE" --trials "$TRIALS" --jobs "$JOBS" --mode "$OPTIM_MODE"
if [ $? -ne 0 ]; then echo -e "\n${RED}Fehler im Optimizer-Skript. Pipeline wird abgebrochen.${NC}"; deactivate; exit 1; fi
