...
from jaegerbot.utils.ann_model import prepare_data_for_ann, load_model_and_scaler
...
def load_data(symbol, timeframe, start_date_str, end_date_str):
    ...
    print(f"Starte Download für {symbol} von der Börse...")
    try:
...
def run_ann_backtest(data, params, model_paths, start_capital=1000):
    ...
    X, _ = prepare_data_for_ann(data.copy())
    ...
    for i in range(len(data_with_features)):
        ...
        if position:
            ...
            if position['side'] == 'long':
                if not position['trailing_active'] and current['high'] >= position['take_profit']: exit_price, reason = position['take_profit'], "Take-Profit"
                elif current['low'] <= position['stop_loss']: exit_price, reason = position['stop_loss'], "Stop-Loss"
                ...
                if position['trailing_active']:
                    ...
                    if current['low'] <= trailing_sl: exit_price, reason = trailing_sl, "Trailing Stop"
            elif position['side'] == 'short':
                if not position['trailing_active'] and current['low'] <= position['take_profit']: exit_price, reason = position['take_profit'], "Take-Profit"
                elif current['high'] >= position['stop_loss']: exit_price, reason = position['stop_loss'], "Stop-Loss"
                ...
                if position['trailing_active']:
                    ...
                    trailing_sl = position['peak_price'] * (1 + callback_rate)
                    if current['high'] >= trailing_sl: exit_price, reason = trailing_sl, "Trailing Stop"
            if exit_price:
